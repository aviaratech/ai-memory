-- Reconcile the verified example/catalog basename only. Unqualified "catalog" is not
-- repository evidence. Keep immutable memory IDs, keys, source payloads and times.
  ...
  AND NOT EXISTS (
    ...
    WHERE m.session_id = s.session_id
      AND ((m.repo_slug IS NOT NULL AND m.repo_slug <> 'example/catalog')
        OR (m.repo_id IS NOT NULL AND m.repo_id NOT IN ('catalog', 'example/catalog'))
...
WHERE project = 'catalog'
  ...
  AND (repo_id IS NULL OR repo_id IN ('catalog', 'example/catalog'));
...
BEGIN
  IF EXISTS (
    ...
    RAISE EXCEPTION 'Verified project identity checkpoint collision: reconcile the ai and example/catalog scoped checkpoints before retrying migration 005';
  END IF;
END $$;
